...
import com.farm.ibot.api.interact.action.impl.MenuNodeAction;
...
public class MenuInteractHandler implements InteractHandler {
    ...
    public boolean widgetInteract(String action, Widget widget) {
        ...
        Mouse.move(p.x, p.y);
        Debug.log("Widget interact " + action + " " + widget.getId());
    ...
    public boolean menuInteract(String action, Interactable interactable) {
        ...
        if (node != null) {
            MenuNodeAction.create(node).send();
            return true;
        } else {
